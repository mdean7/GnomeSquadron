...
for (var i = 0; i < bossenemyTotal; i++) {
 bossenemies.push([bossenemy_x, bossenemy_y, bossenemy_w, bossenemy_h, speed, bossenemyHP]);
...
for (var i = 0; i < rockTotal; i++) {
 ...
 rock_y = 570;
 if(i % 5 === 0) {rock_y -= Math.random() * 50}
 if(i % 7 === 0){rock_y -=90}
}
//arr for stalagtites
for (var i = 0; i < stalagtiteTotal; i++) {
 stalagtites.push([stalagtite_x, stalagtite_y, stalagtite_w, stalagtite_h, speed]);
 stalagtite_x += stalagtite_w * Math.random() * 5 + 3 * stalagtite_w;
 stalagtite_y = -175;
 if(i % 5 === 0) {stalagtite_y -= Math.random()*100}
 if(i % 7 === 0){stalagtite_y += 90}
}
...
function moveEnemies() {
  ...
  for (var i = 0; i < enemies.length; i++) {
   if (enemies[i][0] >= 0) {
    ...
    if(i % 2 === 0){enemies[i][1] -=1}
     if(i % 2 === 1){enemies[i][0] -= 2;
      enemies[i][1] += 2;
      enemies[i][0] += 12;
      enemies[i][1] -= 2;
      enemies[i][0] += 15;
    ...
    for (var i = 0; i < bossenemies.length; i++) {
      ...
      if (bossenemies[i][0] <= 400){            
        ...
        if(bossenemies[0]){
          bossenemies[0][1] +=53;
          blastSound.play(); 
          bossmagics.push([bossenemies[0][0] - 25, bossenemies[0][1] - 20, bossmagic_w, bossmagic_h])
        }
        if(bossenemies[1]){
          bossenemies[1][1] +=53;
        }
        if(bossenemies[2]){
          bossenemies[2][1] +=13;
          blastSound.play();     
          bossmagics.push([bossenemies[2][0] - 25, bossenemies[2][1] - 20, bossmagic_w, bossmagic_h])
        }    
...
function drawpowerup() {
  for (var i = 0; i < powerups.length; i++) {    
    ctx.fillStyle = '#g00';
    ctx.drawImage(powerup, (powerups[i][0] + powerups[i][2]), powerups[i][1], 100, 100);
 }
...
function hitTest() {
 ...
 for (var i = 0; i < magics.length; i++) {
   for (var j = 0; j < enemies.length; j++) {
     if (!poweredUp && magics[i][0] >= (enemies[j][0]) && magics[i][1] >= enemies[j][1] - 3*enemies[j][3] && magics[i][1] <= (enemies[j][1] + 4*enemies[j][3])) {
      ...
      if ( poweredUp && magics[i][0] >= (enemies[j][0]) && magics[i][1] >= enemies[j][1] - 6*enemies[j][3] && magics[i][1] <= (enemies[j][1] + 8*enemies[j][3])) {
...
function gnomeCollision() {
  ...
  for (var i = 0; i < enemies.length; i++) {
   if (gnome_xw > enemies[i][0] && gnome_x < enemies[i][0] + enemy_w && gnome_yh > enemies[i][1] && gnome_y < enemies[i][1] + enemy_h) {
    //  ouchSound.play();
     checkLives();
    }
    if (gnome_xw < enemies[i][0] + enemy_w && gnome_xw > enemies[i][0] && gnome_y > enemies[i][1] && gnome_y < enemies[i][1] + enemy_h) {
      // ouchSound.play();
     checkLives();
    }
    if (gnome_yh > enemies[i][1] && gnome_y < enemies[i][1] + enemy_h && gnome_x > enemies[i][0] && gnome_x < enemies[i][0] + enemy_w) {
      // ouchSound.play();
     checkLives();
    }
    if (gnome_yh > enemies[i][1] && gnome_yh < enemies[i][1] + enemy_h && gnome_xw < enemies[i][0] + enemy_w && gnome_xw > enemies[i][0]) {
      // ouchSound.play();
     checkLives();
    }
  }
  ...
  for (var i = 0; i < bossenemies.length; i++) {
   if (gnome_xw > bossenemies[i][0] && gnome_x < bossenemies[i][0] + bossenemy_w && gnome_yh > bossenemies[i][1] && gnome_y < bossenemies[i][1] + bossenemy_h) {
    //  ouchSound.play();
     checkLives();
    }
    if (gnome_xw < bossenemies[i][0] + bossenemy_w && gnome_xw > bossenemies[i][0] && gnome_y > bossenemies[i][1] && gnome_y < bossenemies[i][1] + bossenemy_h) {
      // ouchSound.play();
     checkLives();
    }
    if (gnome_yh > bossenemies[i][1] && gnome_y < bossenemies[i][1] + bossenemy_h && gnome_x > bossenemies[i][0] && gnome_x < bossenemies[i][0] + bossenemy_w) {
      // ouchSound.play();
     checkLives();
    }
    if (gnome_yh > bossenemies[i][1] && gnome_yh < bossenemies[i][1] + bossenemy_h && gnome_xw < bossenemies[i][0] + bossenemy_w && gnome_xw > bossenemies[i][0]) {
      // ouchSound.play();
     checkLives();
    }
  }
  for (var i = 0; i < bossmagics.length; i++) {
   if (gnome_xw > bossmagics[i][0] && gnome_x < bossmagics[i][0] + bossmagic_w && gnome_yh > bossmagics[i][1] && gnome_y < bossmagics[i][1] + bossmagic_h) {
     checkLives();
    }
    if (gnome_xw < bossmagics[i][0] + bossmagic_w && gnome_xw > bossmagics[i][0] && gnome_y > bossmagics[i][1] && gnome_y < bossmagics[i][1] + bossmagic_h) {

     checkLives();
    }
    if (gnome_yh > bossmagics[i][1] && gnome_y < bossmagics[i][1] + bossmagic_h && gnome_x > bossmagics[i][0] && gnome_x < bossmagics[i][0] + bossmagic_w) {

     checkLives();
    }
    if (gnome_yh > bossmagics[i][1] && gnome_yh < bossmagics[i][1] + bossmagic_h && gnome_xw < bossmagics[i][0] + bossmagic_w && gnome_xw > bossmagics[i][0]) {
   ...
   for (var i = 0; i<rocks.length; i++){
    if (gnome_x > rocks[i][0] && gnome_x < rocks[i][0] + rock_w && gnome_y > rocks[i][1] && gnome_y < rocks[i][1] + rock_h) {
     ramSound.play();
      checkLives();
    }
    if (gnome_xw < rocks[i][0] + rock_w && gnome_xw > rocks[i][0] && gnome_y > rocks[i][1] && gnome_y < rocks[i][1] + rock_h) {
    ramSound.play();
      checkLives();
    }
    if (gnome_yh > rocks[i][1] && gnome_yh < rocks[i][1] + rock_h && gnome_x > rocks[i][0] && gnome_x < rocks[i][0] + rock_w) {
     ramSound.play();
      checkLives();
    }
    if (gnome_yh > rocks[i][1] && gnome_yh < rocks[i][1] + rock_h && gnome_xw < rocks[i][0] + rock_w && gnome_xw > rocks[i][0]) {
     ramSound.play();
      checkLives();
    }
  }
  for (var i = 0; i<stalagtites.length; i++){
    if (gnome_x > stalagtites[i][0] && gnome_x < stalagtites[i][0] + stalagtite_w && gnome_y < stalagtites[i][1] && gnome_y < stalagtites[i][1] + stalagtite_h) {
    ramSound.play();
      checkLives();
    }
    if (gnome_xw < stalagtites[i][0] + stalagtite_w && gnome_xw > stalagtites[i][0] && gnome_y < stalagtites[i][1] && gnome_y < stalagtites[i][1] + stalagtite_h) {
    ramSound.play();
      checkLives();
    }
    if (gnome_y < stalagtites[i][1] + stalagtite_h && gnome_x > stalagtites[i][0] && gnome_x < stalagtites[i][0] + stalagtite_w) {
    ramSound.play();
      checkLives();
    }
    if (gnome_y < stalagtites[i][1] + stalagtite_h && gnome_xw < stalagtites[i][0] + stalagtite_w && gnome_xw > stalagtites[i][0]) {
    ramSound.play();
      checkLives();
    }
  }
  for (var i = 0; i < powerups.length; i++) {
   if (gnome_xw > powerups[i][0] && gnome_x < powerups[i][0] + powerup_w && gnome_yh > powerups[i][1] && gnome_y < powerups[i][1] + powerup_h) {
    ...
    if (gnome_xw < powerups[i][0] + powerup_w && gnome_xw > powerups[i][0] && gnome_y > powerups[i][1] && gnome_y < powerups[i][1] + powerup_h) {
      powerupSound.play()
     poweredUp=true;
    }
    if (gnome_yh > powerups[i][1] && gnome_y < powerups[i][1] + powerup_h && gnome_x > powerups[i][0] && gnome_x < powerups[i][0] + powerup_w) {
      powerupSound.play()
     poweredUp=true;
    }
...
function continueButton(e) {
  ...
  if (cursorPos.x > (width / 2) - 86 && cursorPos.x < (width / 2) + 67 && cursorPos.y > (height / 2) - 10 && cursorPos.y < (height / 2) + 70) {
    continueSound.play();  
    alive = true;
    lives = 3;
    score = 0;
